...
func (c *Conn) serve() {
	...
	defer func() {
		e := recover()
		if e != nil {
			logger.Log.Debugf("Client disconnect: %v", e)
		}
	}()
	...
	ver := string(sl)
	logger.Log.Debugf("client wants: %q", ver)
	switch ver {
	case v3, v7, v8: // cool.
	default:
	...
	logger.Log.Debugf("reading client init")
	...
	for {
		//logger.Log.Debugf("awaiting command byte from client...")
		cmd := c.readByte("6.4:client-server-packet-type")
		//logger.Log.Debugf("got command type %d from client", int(cmd))
		switch cmd {
...
func (c *Conn) pushFrame(ur FrameBufferUpdateRequest) {
	...
	if ur.incremental() {
		...
		//logger.Log.Debugf("Client wants incremental update, sending none. %#v", ur)
...
func (c *Conn) pushImage(li *LockableImage) {
	...
	if b.Min.X != 0 || b.Min.Y != 0 {
		logger.Log.Errorf("this code is lazy and assumes images with Min bounds at 0,0")
		return
	}
	width, height := b.Dx(), b.Dy()
	...
	//logger.Log.Debugf("sending %d x %d pixels", width, height)
...
func (c *Conn) pushGenericLocked(im image.Image) {
	...
	for y := 0; y < height; y++ {
		for x := 0; x < width; x++ {
			col := im.At(x, y)
			// 将一个像素点从RGBA转换成RGB，然后转换成一个字节来发送
			// 其它类型比如NRGBA有不同的转换成RGB的方法
...
func (c *Conn) handleSetPixelFormat() {
	logger.Log.Debugf("handling setpixel format")
	...
	logger.Log.Debugf("Client wants pixel format: %#v", pf)
	c.format = pf
...
func (c *Conn) handleSetEncodings() {
	...
	logger.Log.Debugf("Client encodings: %#v", encType)
...
func inRange(v uint32, max uint16) uint32 {
	...
	logger.Log.Errorf("unsupported inRange: v=%d, max=%d", v, max)
	return 0
}
